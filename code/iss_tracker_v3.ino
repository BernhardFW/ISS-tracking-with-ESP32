...
// --- Page cycling ---
static uint8_t pageIndex = 0;  // 0 = LIVE, 1 = NEXT PASS, 2 = LOCATION

// --- Location name caching ---
unsigned long lastLocPoll = 0;
const unsigned long locPollMs = 15000;  // refresh location every 15s
String locName = "--";
...
// ---- DROP-IN: reverse-geocode using BigDataCloud (Arduino_JSON safe) ----
bool fetchLocationName(double lat, double lon, String& nameOut) {
  String url = "https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=" + String(lat, 6) + "&longitude=" + String(lon, 6) + "&localityLanguage=en";

  WiFiClientSecure client;
  client.setInsecure();  // skip TLS CA check on ESP32
  HTTPClient http;
  if (!http.begin(client, url)) return false;
  ...
  auto getStr = [](JSONVar parent, const char* key) -> String {
    if (parent.hasOwnProperty(key)) {
      JSONVar v = parent[key];
      if (JSON.typeof(v) == "string") {
        const char* s = (const char*)v;
        if (s && *s) return String(s);
      }
    }
    return String("");
  };

  String city = getStr(obj, "city");
  String local = getStr(obj, "locality");
  String subdiv = getStr(obj, "principalSubdivision");
  String country = getStr(obj, "countryName");

  // Try to detect oceans/seas via localityInfo.informative[].type
  String water = "";
  if (obj.hasOwnProperty("localityInfo")) {
    JSONVar info = obj["localityInfo"];
    if (info.hasOwnProperty("informative")) {
      JSONVar arr = info["informative"];
      int n = (int)arr.length();
      for (int i = 0; i < n; ++i) {
        JSONVar it = arr[i];
        String type = getStr(it, "type");
        String name = getStr(it, "name");
        if (type.length() && name.length()) {
          if (type == "ocean" || type == "sea" || type == "waterbody") {
            water = name;
            break;
          }
        }
  ...
  String best;
  if (water.length()) best = water;  // e.g., "Pacific Ocean"
  else if (city.length()) best = city + (country.length() ? "/" + country : "");
  else if (local.length()) best = local + (country.length() ? "/" + country : "");
  else if (subdiv.length() && country.length()) best = subdiv + "/" + country;
  else if (country.length()) best = country;
  else best = "Unknown";
...
void loop() {
  ...
  // pageIndex is a global (0=NOW, 1=NEXT, 2=LOCATION)
  ...
  // ---- Refresh the location name occasionally (uses current ISS coords) ----
  if (millis() - lastLocPoll > locPollMs) {
    String nm;
    if (fetchLocationName(issLat, issLon, nm)) {
      locName = nm;
    }
    lastLocPoll = millis();
  }
  ...
  // ---- Alternate LCD pages every ~4 seconds (0,1,2) ----
  if (millis() - lastPageFlip > 4000) {
    pageIndex = (pageIndex + 1) % 3;  // 0=NOW, 1=NEXT, 2=LOCATION
    lastPageFlip = millis();
  }
  ...
  if (pageIndex == 0) {
    ...
    // ---- Page 3: LOCATION NAME ----
    lcd.setCursor(0, 0);
    lcd.print("Loc:");
    String first = locName;

    if (first.length() > 12) {
      String row1 = first.substring(0, 12);
      String row2 = first.substring(12);
      lcd.print(row1);
      lcd.setCursor(0, 1);
      ...
      lcd.print(first);
      lcd.setCursor(0, 1);
      // optional: show compact coords
      char buf[17];
      snprintf(buf, sizeof(buf), "%+.1f,%+.1f", issLat, issLon);
      buf[16] = '\0';
      lcd.print(buf);
    }
  }
